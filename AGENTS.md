...
A project prospecting tracker that turns forwarded emails, pasted messages, and bid documents into a structured chronology of customer needs, requirement changes, and vendor interactions — so every project stays clear and refreshable at a glance.
...
- **Commit as your GitHub identity, or Vercel will block the deploy.** Vercel verifies that
  ...
  ```
  git config user.email "241380634+eemp-ux@users.noreply.github.com"
  git config user.name "eemp-ux"
  ```
